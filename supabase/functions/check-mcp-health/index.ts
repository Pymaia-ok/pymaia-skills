// check-mcp-health v1.0 — URL health check for MCP servers
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!servers || servers.length === 0) {
      return new Response(JSON.stringify({ checked: 0, down: 0 }), {
    ...
    for (const server of servers) {
      ...
      if (allDown) {
        ...
        // Update security_notes with down timestamp
        if (!downMatch) {
        ...
        // If down > 48 hours, create incident
        if (downHours > 48) {
        ...
        if (notes.includes("URL_DOWN_SINCE:")) {
          ...
          // Resolve any open url_down incidents
    ...
    return new Response(JSON.stringify({ checked, down }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
