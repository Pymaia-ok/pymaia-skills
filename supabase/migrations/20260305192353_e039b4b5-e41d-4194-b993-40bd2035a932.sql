DROP FUNCTION IF EXISTS public.search_skills(text,text,text,text[],text,integer,integer);
...
RETURNS TABLE(
  ...
  category text, industry text[], target_roles text[],
  install_command text, github_url text, video_url text,
  time_to_install_minutes integer, install_count integer,
  avg_rating numeric, review_count integer, github_stars integer,
  use_cases jsonb, creator_id uuid, created_at timestamp with time zone,
  status text, similarity_score real, total_count bigint
)
  ...
  scored AS (
    SELECT
      ...
      GREATEST(
        ...
        similarity(lower(c.tagline), query_lower),
        similarity(lower(COALESCE(c.tagline_es, '')), query_lower),
        similarity(lower(c.description_human), query_lower),
        similarity(lower(COALESCE(c.description_human_es, '')), query_lower),
