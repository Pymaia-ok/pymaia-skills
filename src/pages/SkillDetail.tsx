  ...
  const handleSubmitReview = async () => {
    ...
    try {
      await createReview({
        ...
        user_id: user.id,
        rating: reviewRating,
        time_saved: reviewTimeSaved || undefined,
      ...
      toast.error("No se pudo publicar la review. ¿Ya dejaste una?");
    }
    setSubmitting(false);
  };

  const installSteps = [
    { title: "¿Qué es una skill?", description: "Una skill es un archivo que le enseña a Claude cómo hacer una tarea específica de tu trabajo. No es una app ni un plugin — es conocimiento experto que Claude usa automáticamente." },
    { title: "Abrí Claude Code en tu computadora", description: "Si todavía no tenés Claude Code, andá a nuestra guía de primeros pasos para instalarlo en 5 minutos." },
    { title: "Copiá y pegá este comando", description: "En la ventana de Claude Code, pegá el siguiente comando y presioná Enter.", command: skill.install_command },
    { title: "¡Listo! Ya podés usarla", description: `Claude ahora sabe cómo ${skill.tagline.toLowerCase()}. Simplemente pedile que haga la tarea y va a usar la skill automáticamente.` },
  ];
        ...
        <div className="max-w-4xl mx-auto px-6 py-16">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex items-center gap-6 text-sm text-muted-foreground mb-12">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span>{skill.install_count.toLocaleString()} instalaciones</span>
              </div>
              <div className="flex items-center gap-1.5">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-20">
            <div className="flex items-center justify-between mb-8">
              ...
              {user && !showReviewForm && (
                <button onClick={() => setShowReviewForm(true)} className="text-sm font-medium px-4 py-2 rounded-full bg-foreground text-background hover:opacity-90 transition-opacity">
                  Dejar review
                </button>
              )}
              {!user && (
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <button key={n} onClick={() => setReviewRating(n)}>
                        <Star className={`w-5 h-5 ${n <= reviewRating ? "fill-foreground text-foreground" : "text-muted-foreground"}`} />
                      </button>
                    ))}
                  </div>
                </div>
                <div className="mb-4">
                  <label className="text-sm font-medium mb-2 block">¿Cuánto tiempo te ahorra por semana?</label>
                  <input
                    value={reviewTimeSaved}
                    onChange={(e) => setReviewTimeSaved(e.target.value)}
                    placeholder="Ej: 3 horas por semana"
                    className="w-full px-4 py-3 rounded-xl bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring text-sm"
                  />
                </div>
                <div className="mb-4">
                  <label className="text-sm font-medium mb-2 block">Contanos cómo la usás</label>
                  <textarea
                    value={reviewComment}
                    onChange={(e) => setReviewComment(e.target.value)}
                    rows={3}
                    placeholder="¿Qué pudiste hacer con esta skill que antes no podías?"
                    className="w-full px-4 py-3 rounded-xl bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring text-sm resize-none"
                  />
                </div>
                <div className="flex gap-3">
                  <button onClick={handleSubmitReview} disabled={submitting} className="text-sm font-medium px-6 py-2.5 rounded-full bg-foreground text-background hover:opacity-90 transition-opacity disabled:opacity-50">
                    {submitting ? "Publicando..." : "Publicar review"}
                  </button>
                  <button onClick={() => setShowReviewForm(false)} className="text-sm text-muted-foreground hover:text-foreground">
                    Cancelar
                  </button>
                </div>
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
            <div className="border-t border-border pt-16">
              ...
              <p className="text-muted-foreground mb-10">
                Paso a paso, sin terminal. En {skill.time_to_install_minutes} minutos la tenés funcionando.
              </p>
              ...
              <div className="mt-10 p-6 rounded-2xl bg-secondary text-center">
                ...
                <Link to="/primeros-pasos" className="text-sm font-medium text-foreground hover:underline">
