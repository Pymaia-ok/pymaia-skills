  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        toast({
          title: t("auth.checkEmail", "Revisá tu email"),
          description: t("auth.confirmSent", "Te enviamos un link de confirmación."),
        });
      } else {
      ...
      <div className="pt-14 min-h-[calc(100vh-3.5rem)] flex items-center justify-center px-6">
        ...
        >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl border border-border bg-background hover:bg-secondary transition-colors text-sm font-medium"
          >
          ...
          <form onSubmit={handleEmailAuth} className="space-y-3">
            ...
            <input
              ...
              required
              minLength={6}
              placeholder={t("auth.passwordPlaceholder", "Contraseña (mín. 6 caracteres)")}
              value={password}
            ...
            {mode === "login" ? (
              ...
              <>
                ...
                <button onClick={() => setMode("login")} className="underline hover:text-foreground transition-colors">
                  {t("auth.loginLink", "Iniciá sesión")}
                </button>
              </>
            )}
          </p>
