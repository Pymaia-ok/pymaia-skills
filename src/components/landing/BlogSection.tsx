  ...
  const { data: posts } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("blog_posts")
        .select("slug, title, title_es, excerpt, excerpt_es, category, reading_time_minutes, created_at, keywords")
        .eq("status", "published")
            ...
            <Link key={post.slug} to={`/blog/${post.slug}`}>
              <Card className="h-full hover:shadow-md transition-shadow border-border/50 group">
                <CardContent className="p-6 flex flex-col h-full">
                  ...
                  <h3 className="text-lg font-semibold text-foreground group-hover:text-primary/80 transition-colors mb-3 line-clamp-2">
                  ...
                  <div className="mt-4 flex flex-wrap gap-1">
                    {(post.keywords || []).slice(0, 2).map((kw: string) => (
                      <span key={kw} className="text-xs text-muted-foreground/70 bg-muted px-2 py-0.5 rounded">
                        {kw}
                      </span>
                    ))}
                  </div>
